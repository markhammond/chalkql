...
/// <summary>
/// The windows-II corpus against the reference executor (D55–D60, D66–D68,
/// <c>14-windows-ii.md</c> §9).
///
/// <para>
/// A class of its own for the reason the window corpus has one: the reference executor materialises
/// every frame and expands every list, so these queries run against <c>bars_small</c>,
/// <c>symbols</c> and <c>trades_sparse</c> rather than the full-size tables. It is also where the
/// row-scanned baseline would say the wrong thing — a session window's required ordering turns the
/// scan into an index-ordered lookup that still reads every row.
/// </para>
/// </summary>
[Collection(SidecarCollection.Name)]
public sealed class WindowsIiDifferentialTests(SharedSidecar sidecar)
{
    /// <summary>The corpus family this class owns; <see cref="CorpusDifferentialTests"/> skips it.</summary>
    public const string Milestone = "m5-windows-ii";
    ...
    public static TheoryData<string> Queries()
    {
        ...
        foreach (var query in CorpusQueries.LoadM5())
        {
            data.Add(query.Name);
        }
    ...
    /// <summary>
    /// §10's arena claim: every query of this family leaves the arena empty. The hop, session and
    /// unnest operators rent their gather positions and their buffered input, the holistic
    /// aggregates rent their whole value store, and every one of them comes back in the run's
    /// <c>finally</c>.
    /// </summary>
    [Theory]
    [MemberData(nameof(Queries))]
    public async Task A_windows_ii_query_leaves_the_arena_empty(string name)
    {
        Assert.SkipWhen(!sidecar.Sidecar.IsAvailable, sidecar.SkipReason ?? string.Empty);
    ...
    /// <summary>
    /// The same comparison at a batch size no operator's emission loop can swallow: a hop's copies
    /// of one row, and a list's elements, then straddle batch boundaries, and an operator that lost
    /// track of which input row it was expanding would show up as a shifted column.
    /// </summary>
    [Theory]
    [MemberData(nameof(Queries))]
    public async Task Batch_boundaries_do_not_move_a_row(string name)
    {
        Assert.SkipWhen(!sidecar.Sidecar.IsAvailable, sidecar.SkipReason ?? string.Empty);
    ...
    public async Task The_corpus_takes_the_streaming_path_where_the_frame_allows()
    {
        ...
        // Twelve since 27_count_over_a_large_magnitude_double joined the family: a COUNT under a
        // ROWS frame that ends at the current row, so it streams.
        Assert.True(windows == 12, tally);
        Assert.True(streamed.Count == 5, tally);
        Assert.True(buffered.Count == 7, tally);
    }

    private static CorpusQuery Query(string name) =>
        CorpusQueries.LoadM5().Single(q => q.Name == name);
