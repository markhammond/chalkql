...
public sealed class AdoSourceBuilder
{
    ...
    /// </summary>
    public AdoSourceBuilder(string sourceId, Func<DbConnection> connect, string schemaName = "main")
    {
    ...
    private static Func<DbConnection> ConnectionFrom(DbProviderFactory factory, string connectionString)
    {
    ...
    /// <summary>How this source spells and evaluates SQL. One of <see cref="DialectProfiles"/>, adjusted.</summary>
    ...
    /// <summary>
    /// Registers one table explicitly, overriding whatever discovery would have said about it. The
    /// escape hatch for a provider whose schema collections are thin, or a column whose declared
        ...
        Action<AdoTableBuilder>? configure = null)
    {
        ...
        if (columns.Count == 0)
        {
            throw new ArgumentException($"Table '{name}' needs at least one column.", nameof(columns));
        }
    ...
    /// <c>docs/design/44-catalog-registration.md</c> §6 (h)): the source carried with the name, so a
    /// scoped refresh cannot reach a same-named table on another source.
    /// </summary>
    /// <remarks>
    /// An <c>out</c> parameter rather than a different return type, so the builder's fluent chain
    /// still runs through it. A discovered table has no registration to take a handle at;
    /// <see cref="AdoSource.Table(string)"/> is where its handle comes from.
    /// </remarks>
    public AdoSourceBuilder AddTable(
        string name,
        IReadOnlyList<ColumnDescriptor> columns,
    ...
    /// </summary>
    /// <remarks>
    /// Separate from <see cref="AddTable"/> because a remote table is usually <em>discovered</em>:
    /// the shape comes from the provider and the policy from the host, and asking a host to hand-write
    /// a discovered table's columns in order to attach an entitlement to it would be a way to get the
    /// shape wrong. Naming a table this source does not have is a registration error rather than a
    ...
    public AdoSourceBuilder Entitlement(string table, TableEntitlementDescriptor entitlement)
    {
    ...
    /// </summary>
    public AdoSourceBuilder AddFunction(string name, Action<FunctionBuilder> configure)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(name);
    ...
    public AdoSource Build()
    {
        ...
        var native = _functions
            ...
            .Select(f => f.Name)
            .Where(n => !_capabilities.NativeFunctions.Contains(n, StringComparer.OrdinalIgnoreCase))
            .ToArray();
        if (native.Length > 0)
        {
        ...
        var source = new AdoSource(
            ...
            tables,
            _costProfile,
            _discover.Count > 0 || _rowCounts != RowCountMode.Unknown ? introspect : null,
            [.. _functions],
            _fetch,
            _traits,
    ...
    /// <summary>
    /// The tables as the database has them now: the hand-registered ones, then whatever discovery
    /// finds that they did not already name, then exact row counts if they were asked for.
    /// </summary>
    private AdoTableDefinition[] Introspect()
    {
        var registrations = new List<TableRegistration>(_tables);
        if (_discover.Count > 0 || _rowCounts != RowCountMode.Unknown)
        {
            ...
            {
                foreach (var discovered in AdoDiscovery.Tables(connection, schema, _profile))
                {
                    if (!registrations.Any(t => string.Equals(
                        t.Descriptor.Name, discovered.Descriptor.Name, StringComparison.OrdinalIgnoreCase)))
                    {
            ...
            if (_rowCounts == RowCountMode.Exact)
            {
                ...
                {
                    var count = AdoDiscovery.Count(connection, registrations[i].RemoteName, _profile);
                    registrations[i] = registrations[i] with
                    {
                        Descriptor = With(registrations[i].Descriptor, count, RowCountKind.Exact),
                    };
                }
            }
        }
        ...
        return registrations.Select(r => new AdoTableDefinition(r.Descriptor, r.RemoteName)).ToArray();
    }
    ...
    private void Entitle(List<TableRegistration> registrations)
    {
        ...
        for (var i = 0; i < registrations.Count; i++)
        {
            ...
            if (!_entitlements.TryGetValue(descriptor.Name, out var entitlement))
            {
                continue;
            }
    ...
    private void ResolveForeignKeys(List<TableRegistration> registrations)
    {
        for (var t = 0; t < registrations.Count; t++)
        {
            ...
            var resolved = new List<ForeignKeyDescriptor>(registration.ForeignKeys.Count);
            foreach (var key in registration.ForeignKeys)
            {
                var parent = registrations.Find(r => string.Equals(
                    r.Descriptor.Name, key.ParentTable, StringComparison.OrdinalIgnoreCase))
                ...
                {
                    parentColumns[i] = parent.Descriptor.IndexOfColumn(key.ParentColumns[i]);
            ...
            {
                Descriptor = WithForeignKeys(registration.Descriptor, resolved),
            };
        }
    }

    private static TableDescriptor WithForeignKeys(
        TableDescriptor table, IReadOnlyList<ForeignKeyDescriptor> keys) => new()
    {
        Name = table.Name,
    ...
    private static TableDescriptor With(TableDescriptor table, long rowCount, RowCountKind kind) => new()
    {
        Name = table.Name,
        Columns = table.Columns,
        RowCount = rowCount,
...
internal sealed record DeclaredForeignKey(
    string Name, int[] Columns, string ParentTable, string[] ParentColumns);

/// <summary>Keys, indexes and statistics for a table registered by hand.</summary>
public sealed class AdoTableBuilder
{
    ...
    internal AdoTableBuilder(string name, IReadOnlyList<ColumnDescriptor> columns, string remoteName)
    {
        _name = name;
        _columns = columns;
    ...
    public AdoTableBuilder UniqueKey(params string[] columns)
    {
        ArgumentNullException.ThrowIfNull(columns);
        _uniqueKeys.Add(new UniqueKeyDescriptor { Columns = Resolve(columns) });
        return this;
    }
    ...
    /// </summary>
    public AdoTableBuilder Index(string name, bool unique, params string[] columns)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(name);
        ArgumentNullException.ThrowIfNull(columns);
    ...
    public AdoTableBuilder ForeignKey(string name, string[] columns, string parentTable, string[] parentColumns)
    {
        ArgumentNullException.ThrowIfNull(columns);
        ...
        _foreignKeys.Add(new DeclaredForeignKey(name, Resolve(columns), parentTable, parentColumns));
        return this;
    }

    /// <summary>How many rows the table holds, and how much that number is worth (D36).</summary>
    public AdoTableBuilder RowCount(long rows, RowCountKind kind = RowCountKind.Estimate)
    {
        _rowCount = rows;
        _rowCountKind = kind;
        return this;
    ...
    private int[] Resolve(string[] columns)
    {
        ...
        for (var i = 0; i < columns.Length; i++)
        {
            ...
            {
                if (string.Equals(_columns[c].Name, columns[i], StringComparison.OrdinalIgnoreCase))
                {
                    found = c;
                    break;
                }
