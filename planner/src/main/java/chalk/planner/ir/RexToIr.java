...
public final class RexToIr extends RexVisitorImpl<Expr> {
  ...
  public Expr visitCall(RexCall call) {
    ...
    switch (call.getKind()) {
      ...
      case ROW, MAP_VALUE_CONSTRUCTOR ->
          throw new UnsupportedFeatureException(
              "constructor " + call.getKind(),
              "The IR's only composite value type is LIST (docs/design/14-windows-ii.md §5).");
      default -> {
  ...
  public Expr visitFieldAccess(RexFieldAccess access) {
    throw unsupported("field access", "The IR has no composite value types in v1.");
  }
